...
console.log(capsule.toBytes())   //this is the transform key I guess
...
/*
The thing which needs to be done is that we have to explain what happens in this code and how we need to get these values or store these
values for using in ymca. Basically the re-encryption and things should be saved for future things and these values should be written using 
wasm. Basically we must create a javascript server, from wehre ymca should be able to call the api-endpoints and in these api calls must
return the keys or the encrypted function or decrypt functions should be triggered. Also this server should be able to communicate with
the rubix node for creating the did, executing the contract and things like that. This needs to be done.
*/